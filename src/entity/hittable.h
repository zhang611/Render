...
// leading to numerous "使用了未定义类型 hit_record" errors in MSVC.
...
public:
	...
	inline void set_face_normal(const ray& r, const vec3& outward_normal)
	{
		...
		front_face = dot(r.direction(), outward_normal) < 0; // �������棬��ķ���ͷ��߷���нǴ���90��Ϊ����
	...
	rotate_y(shared_ptr<hittable> object, double angle) : object(object)
	{
		...
		for (int i = 0; i < 2; i++)
		{
			for (int j = 0; j < 2; j++)
			{
				for (int k = 0; k < 2; k++)
				{
					auto x = i * bbox.x.max_ + (1 - i) * bbox.x.min_;
					auto y = j * bbox.y.max_ + (1 - j) * bbox.y.min_;
					auto z = k * bbox.z.max_ + (1 - k) * bbox.z.min_;
